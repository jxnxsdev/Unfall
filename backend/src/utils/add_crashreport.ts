...
export async function addCrashReport(crash: dataTypes.processed_crash) {
    ...
    const player = await conn.query('SELECT * FROM players WHERE name = ?', [player_id]).catch(console.error);
    // @ts-ignore
    if (player.length === 0) {
    ...
    const cause = JSON.parse(data).cause;

    // check if cause exists
    const causeData = await conn.query('SELECT * FROM crash_causes WHERE cause = ?', [cause]).catch(console.error);
    // @ts-ignore
